...
def face_swap(img_dst, img_cover, landmarks_dst):
    ...
    return cv2.seamlessClone(result, img_dst, face_mask_255, center_face, cv2.NORMAL_CLONE)


def dlib_face_swap(img_dst, img_src):
    # 创建人脸检测器
    det_face = dlib.get_frontal_face_detector()

    # 加载标志点检测器
    det_landmarks = dlib.shape_predictor("models/shape_predictor_68_face_landmarks_GTX.dat")  # 68点
    ...
    # 进行人脸替换
    return face_swap(img_dst, img_cover, landmarks_dst)


if __name__ == "__main__":
    img_src = cv2.imread("./images/JayChou.png")
    img_tgt = cv2.imread("./images/targetImg.jpg")
    result = dlib_face_swap(img_src, img_tgt)
